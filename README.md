...
| `POST /v1/timeentry`                | yes (`authKey`) | Create a time entry. Body: `teCustId` (required), `teStartedAt` (required, ISO 8601), `teEndedAt` (optional — in-flight entries allowed), `teDescription`, `teBillable` (default true), and the optional `teJobId` / `teWorkerId` / `teBillTypeId` relationships (each scoped to the caller's company on write). `teMinutes` is computed server-side on close. |
...
| `PATCH /v1/timeentry/:id`           | yes (`authKey`) | Partial update. Updatable: `teDescription`, `teStartedAt`, `teEndedAt`, `teBillable`, `teJobId`, `teWorkerId`, `teBillTypeId` (a `null` detaches a relationship). `teMinutes` is recomputed on bound change. |
| `DELETE /v1/timeentry/:id`          | yes (`authKey`) | Soft-delete (sets `teArch = true`). Entries are never physically removed via the API. |
| `GET /v1/report/invoice-list`       | yes (`authKey`) | Invoice-list report (restores the source `v_InvoiceList` view): one row per invoice line — `invoiceDate`, `invoiceNumber`, `invoiceAmount`, `customerId` — joining Invoices × Customers × InvoiceJobs. Company-scoped (master keys pass `companyId`); paginated. Query params: `companyId` (master-only), `customerId` (filter), `limit` (default 100, max 500), `offset`. |
| `GET /v1/report/invoice-list.csv`   | yes (`authKey`) | Same projection as a CSV download. Same auth contract + CSV-injection guard as the other `export.csv` endpoints. `limit` default 5000, max 5000. |
